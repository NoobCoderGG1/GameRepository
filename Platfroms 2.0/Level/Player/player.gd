...
onready var Velocity : Vector2 = Vector2() #Для движения персонажем
...
func _physics_process(delta):
	if HP <= 0 and is_Alive:
		...
		var total_time = "%02d" % timerNode.tmin + ":" + "%02d" % timerNode.sec
...
#Заносит врагов в список, если они попали в область действия оружия ближнего боя
